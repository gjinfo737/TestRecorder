...
public class TestRecorderActivityPresenter {
    ...
    public static final String SHARED_PREFERENCE_NAME = TestRecorderActivity.class
	    .getName() + ".prefs";
    ...
    public TestRecorderActivityPresenter(ITestRecorderView c) {
	this.mView = c;
    }
	...
	PackageInfo pInfo;
	try {
	    pInfo = mView.getPackageManager().getPackageInfo(
		    mView.getApplicationContext().getPackageName(), 0);

	    mView.setTextViewText(R.id.textView_version, "Ver."
		    + pInfo.versionName + "." + pInfo.versionCode);
	} catch (NameNotFoundException e) {
	    e.printStackTrace();
	}
	mView.setClickListenerOnView(R.id.button_reload_logs,
		new OnClickListener() {
		    @Override
	...
	mView.setClickListenerOnView(R.id.button_startTracking,
		new OnClickListener() {
		    @Override
		    public void onClick(View v) {
	...
	mView.setClickListenerOnView(R.id.button_stopTracking,
		new OnClickListener() {
		    @Override
		    public void onClick(View v) {
	...
	mView.setClickListenerOnView(R.id.button_savePrefs,
		new OnClickListener() {
		    @Override
		    public void onClick(View v) {
			handleSavePrefsBtn();
	...
	reloadLogs();
	Toast.makeText(mView.getApplicationContext(), "Done reloading logs",
	...
	TestRecorderHelper.takeSnapShot(context.getApplicationContext()
		.getPackageName(), 31);
	...
	TestRecorderHelper.contextName = sharedPrefs.getString(
		SPKEY_CONTEXT_NAME, TestRecorderHelper.DEFAULT_CONTEXT_NAME);
	String ssl = sharedPrefs.getString(SPKEY_SNAP_LENGTH,
		TestRecorderHelper.DEFAULT_SNAPSHOT_LENGTH + "");
	TestRecorderHelper.snapshotLength = Integer.parseInt(ssl);
	TestRecorderHelper.folderName = sharedPrefs.getString(
		SPKEY_FOLDER_NAME, TestRecorderHelper.DEFAULT_FOLDER_NAME);
	TestRecorderHelper.logLevel = LogLevel.valueOf(sharedPrefs.getString(
		SPKEY_LEVEL, TestRecorderHelper.DEFAULT_LEVEL.toString()));
	...
	CharSequence contentText = TestRecorderHelper.snapshotLength + " - "
		+ TestRecorderHelper.folderName;
	...
	notification.flags = Notification.FLAG_NO_CLEAR
		| Notification.FLAG_INSISTENT | Notification.FLAG_ONGOING_EVENT;

	return notification;
    }

    public void savePreferences(String key, String value) {
	SharedPreferences sharedPreferences = mView.getSharedPreferences(
	...
	return mView.getSharedPreferences(SHARED_PREFERENCE_NAME,
